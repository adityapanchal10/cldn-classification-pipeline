...
def compute_ig_attributions(
    model, inputs, baseline, target_class, n_steps=50, device=None
):
...
def get_top_k_residues(residue_attributions, k=10):
    ...
    for i in range(batch_size):
        ...
        neg_values, neg_indices = torch.topk(
            residue_attributions[i], k // 2, largest=False
        )
        results.append(
            {
                "sample_id": i,
                "top_residue_indices": torch.cat((pos_indices, neg_indices))
                .cpu()
                .tolist(),
                "top_residue_scores": torch.cat((pos_values, neg_values))
                .cpu()
                .tolist(),
...
def print_results(results):
    ...
    print(
        f"  Mean convergence delta: {results['summary']['mean_convergence_delta']:.6f}"
    )
    print(
        f"  IG approximation quality: {results['summary']['ig_approximation_quality']}"
    )
...
def visualize_sequence_explanations(
    ...
    for i in range(num_to_plot):
        ...
        true_label = (
            ...
            if (class_names and true_class is not None)
            else None
        )
