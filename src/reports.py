...
def _grouped_composition_summary_df(grouped_results):
    ...
    for r in grouped_results:
        ...
        for cls in [0, 1, 2]:
            row[f"True prop class {cls}"] = comp_df.loc[
                comp_df["class_idx"] == cls, "true_prop"
            ].values[0]
            row[f"Pred prop class {cls}"] = comp_df.loc[
                comp_df["class_idx"] == cls, "pred_prop"
            ].values[0]
            row[f"Abs prop error class {cls}"] = comp_df.loc[
                comp_df["class_idx"] == cls, "abs_prop_error"
            ].values[0]
...
def report_final_training_diagnostics(
    ...
    print("=" * 78)
    print(
        "⚠️ The in-sample macro F1 and AUC are expected to be high (possibly near-perfect) since there's no held-out data — these numbers should not be interpreted as generalisation performance."
    )
    ...
    axes[0].axvline(
        best_epoch, color="gray", linestyle=":", linewidth=1.2, label="Saved epoch"
    )
    ...
    if save_path:
        ...
        plt.savefig(
            f"{save_path}/final_diagnostics_curves.png", dpi=300, bbox_inches="tight"
        )
    plt.show()
    ...
    if show_confusion:
        ...
        if save_path:
            os.makedirs(save_path, exist_ok=True)
            plt.savefig(
                f"{save_path}/final_diagnostics_confusion.png",
                dpi=300,
                bbox_inches="tight",
            )
        plt.show()
...
def _lofo_class_summary_df(fold_results, class_names):
    ...
    for cname in class_names:
        ...
        recalls = [
            r["best_recall"] for r in class_folds if not np.isnan(r["best_recall"])
        ]
        confs = [
            r["best_confidence"]
            for r in class_folds
            if not np.isnan(r["best_confidence"])
        ]
        rows.append(
            {
                "Class": cname,
...
def _lofo_oof_summary_df(fold_results, class_names):
    ...
    y_prob_all = np.concatenate(
        [np.asarray(fr["y_prob"]) for fr in fold_results], axis=0
    )
...
def report_lofo_cv_main(
    ...
    print(
        "Each sample contributes only via out-of-fold predictions from a held-out paralog."
    )
    print("=" * 78)
    ...
    if save_path:
        plt.savefig(
            f"{save_path}/lofo_confusion_matrix.png", dpi=300, bbox_inches="tight"
        )
    plt.show()
    ...
    if show_roc:
        ...
        fig, axes = plt.subplots(
            1, n_classes, figsize=(5 * n_classes, 5), sharex=True, sharey=True
        )
        if n_classes == 1:
            axes = [axes]
        ...
        if save_path:
            plt.savefig(
                f"{save_path}/lofo_roc_curves.png", dpi=300, bbox_inches="tight"
            )
        plt.show()
    ...
    if show_training_curves:
        ...
        axes[1].set(
            ...
            xlabel="Epoch",
            ylabel="Held-out Class Confidence (%)",
        )
        axes[1].grid(True, alpha=0.3)
        ...
        if save_path:
            plt.savefig(
                f"{save_path}/lofo_training_curves.png", dpi=300, bbox_inches="tight"
            )
        plt.show()
...
def report_grouped_holdout_main(
    ...
    if save_path:
        plt.savefig(
            f"{save_path}/grouped_confusion_matrix.png", dpi=300, bbox_inches="tight"
        )
    plt.show()
    ...
    if show_roc:
        ...
        if save_path:
            plt.savefig(
                f"{save_path}/grouped_roc_curves.png", dpi=300, bbox_inches="tight"
            )
        plt.show()
    ...
    if show_training_curves:
        ...
        axes[1, 0].set(
            title="Val Accuracy per Fold", xlabel="Epoch", ylabel="Accuracy (%)"
        )
        ...
        if save_path:
            plt.savefig(
                f"{save_path}/grouped_training_curves.png", dpi=300, bbox_inches="tight"
            )
        plt.show()
...
def report_grouped_holdout_compact(grouped_results, save_path=None):
    ...
    if save_path:
        ...
        ckpt_df.to_csv(
            f"{save_path}/grouped_compact_checkpoint_summary.csv", index=False
        )
