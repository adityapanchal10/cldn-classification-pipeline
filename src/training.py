...
def train_classifier(
    ...
    optimizer = torch.optim.AdamW(
        ...
        lr=optimizer_lr,
        weight_decay=optimizer_weight_decay,  # 3e-3 1e-4
    )
...
def print_final_training_summary(
    history, class_names=["barrier", "cation", "anion"], save_path=None, insample=True
):
    ...
    row_best = {
        ...
        "Macro F1 (%)": (
            history["val_macro_f1"][best_epoch - 1]
            if not np.isnan(history["val_macro_f1"][best_epoch - 1])
            else np.nan
        ),
        "Macro AUC (%)": (
            history["val_auc"][best_epoch - 1]
            if not np.isnan(history["val_auc"][best_epoch - 1])
            else np.nan
        ),
    }
    row_final = {
        ...
        "Macro F1 (%)": (
            history["val_macro_f1"][-1]
            if not np.isnan(history["val_macro_f1"][-1])
            else np.nan
        ),
        "Macro AUC (%)": (
            history["val_auc"][-1] if not np.isnan(history["val_auc"][-1]) else np.nan
        ),
    }
...
def summarize_final_in_sample_metrics(
    history, class_names=["barrier", "cation", "anion"], save_path=None, insample=True
):
    eval_label = _eval_label(insample)
...
def final_in_sample_classification_table(
    history, class_names=["barrier", "cation", "anion"], save_path=None, insample=True
):
    eval_label = _eval_label(insample)
    y_true = np.asarray(history["best_val_targets"])
    ...
    if save_path:
        os.makedirs(save_path, exist_ok=True)
        df_full.to_csv(
            f"{save_path}/final_{eval_label.lower()}_classification_table.csv",
            index=False,
        )
    return df_full
...
def plot_final_training_history(history, save_path=None, insample=True):
    ...
    if save_path:
        os.makedirs(save_path, exist_ok=True)
        plt.savefig(
            f"{save_path}/final_training_history.png", bbox_inches="tight", dpi=300
        )
    plt.show()
...
def plot_final_macro_metrics(history, save_path=None, insample=True):
    ...
    axes[1].axvline(best_epoch, color="gray", linestyle=":", linewidth=1)
    axes[1].set(
        title=f"{eval_label} Macro AUC", xlabel="Epoch", ylabel="Macro AUC OvR (%)"
    )
    axes[1].grid(True, alpha=0.3)
    ...
    if save_path:
        os.makedirs(save_path, exist_ok=True)
        plt.savefig(
            f"{save_path}/final_macro_metrics.png", bbox_inches="tight", dpi=300
        )
    plt.show()
...
def plot_final_confusion_matrix(
    ...
    if save_path:
        os.makedirs(save_path, exist_ok=True)
        plt.savefig(
            f"{save_path}/final_confusion_matrix.png", bbox_inches="tight", dpi=300
        )
    plt.show()
