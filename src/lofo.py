...
def compute_honest_val_metrics(
    ...
    confusion_str = (
        ", ".join(f"{class_names[k]}({v})" for k, v in sorted(confusion.items()))
        if confusion
        else "none"
    )
...
def print_epoch_summary(
    ...
    mode_label = (
        f"LOFO — held out: {held_out_file_name}" if is_lofo else "Final Training"
    )
    ...
    if is_lofo:
        ...
        print(
            f"  Val set: {held_out_file_name}({m['n_val']}) sequences — ALL labelled '{held_out_class_name}'"
        )
        print()
        print(f"  Recall on '{held_out_file_name}':          {m['recall']:.1f}%")
        print(
            f"    → Correctly identified {m['recall']:.1f}% of {held_out_file_name} sequences"
        )
        print()
        print(
            f"  Avg confidence in '{held_out_file_name}':  {m['correct_class_conf']:.1f}%"
        )
        print(
            f"    → Model is {m['confidence_level']} (prediction entropy: {m['entropy']:.3f})"
        )
        print()
...
def train_lofo_cv(
    ...
    for r in fold_results:
        recall_str = (
            f"{r['best_recall']:.1f}%" if not np.isnan(r["best_recall"]) else "  n/a "
        )
        conf_str = (
            f"{r['best_confidence']:.1f}%"
            if not np.isnan(r["best_confidence"])
            else "  n/a "
        )
        best_epoch_recall_str = (
            f"{r['best_epoch_recall']:.1f}%"
            if not np.isnan(r["best_epoch_recall"])
            else "  n/a "
        )
        best_epoch_con_str = (
            f"{r['best_epoch_conf']:.1f}%"
            if not np.isnan(r["best_epoch_conf"])
            else "  n/a "
        )
        print(
            f"  {r['fold']:<5} {str(r['held_out_file']):<28} "
...
def print_lofo_class_recall_summary(
    fold_results, class_names=["barrier", "cation", "anion"], save_path=None
):
    rows = []
...
def summarize_aggregated_oof_metrics(
    fold_results, class_names=["barrier", "cation", "anion"], save_path=None
):
...
def aggregated_oof_classification_table(
    fold_results, class_names=["barrier", "cation", "anion"], save_path=None
):
    y_true_all = np.concatenate([np.asarray(fr["y_true"]) for fr in fold_results])
    y_pred_all = np.concatenate([np.asarray(fr["y_pred"]) for fr in fold_results])
...
def plot_oof_confusion_matrix(
    ...
    sns.heatmap(
        cm_display,
        annot=np.round(cm_display, 1) if normalize else cm,
        fmt=fmt,
        cmap=cmap,
    ...
    if save_path:
        plt.savefig(
            f"{save_path}/lofo_confusion_matrix.png", bbox_inches="tight", dpi=300
        )
    plt.show()
...
def plot_lofo_training_curves(fold_results, save_path=None):
    ...
    axes[2].yaxis.set_major_formatter(plt.matplotlib.ticker.LogFormatterSciNotation())
    ...
    if save_path:
        plt.savefig(
            f"{save_path}/lofo_training_curves.png", bbox_inches="tight", dpi=300
        )
    plt.show()
...
def plot_lofo_primary_metric_curve(
    ...
    if save_path:
        plt.savefig(
            f"{save_path}/lofo_primary_metric_curve.png", bbox_inches="tight", dpi=300
        )
    plt.show()
