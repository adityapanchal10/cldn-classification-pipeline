...
def split_records(records: List[Tuple[str, str]], test_fraction: float, seed: int) -> Tuple[List[Tuple[str, str]], List[Tuple[str, str]]]:
    if not records:
        return [], []

    indices = list(range(len(records)))
    random.Random(seed).shuffle(indices)
    test_size = max(1, int(round(len(records) * test_fraction))) if len(records) > 1 else 0
...
def write_msa_artifact(output_dir: Path, fasta_path: Path, records: List[Tuple[str, str]], embedding: torch.Tensor, model_name: str) -> Path:
...
def write_independent_artifact(output_dir: Path, fasta_path: Path, records: List[Tuple[str, str]], embedding: torch.Tensor, model_name: str) -> Path:
    output_dir.mkdir(parents=True, exist_ok=True)
    artifact_path = output_dir / f"{fasta_path.stem}.pt"
    payload = {
        "sequence_ids": [seq_id for seq_id, _ in records],
...
def write_split_artifact(output_path: Path, source_name: str, split_name: str, records: List[Tuple[str, str]], embedding: torch.Tensor, model_name: str, embedding_mode: str) -> Path:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cache ESM embeddings to Drive in a notebook-aligned format.")
    parser.add_argument("--input", required=True, help="FASTA file or directory containing FASTA files.")
    parser.add_argument("--output-root", required=True, help="Root directory where embeddings will be saved.")
    parser.add_argument("--embedding-mode", choices=["msa", "independent"], required=True)
...
def main() -> None:
    ...
    for fasta_path in fasta_files:
        ...
        if input_path.is_file():
            train_records, test_records = split_records(records, test_fraction=args.test_fraction, seed=args.seed)

            if args.embedding_mode == "msa":
                train_embedding = embedder.embed_msa([seq for _, seq in train_records], seq_length=args.seq_length, max_msa_depth=args.max_msa_depth)
                test_embedding = embedder.embed_msa([seq for _, seq in test_records], seq_length=args.seq_length, max_msa_depth=args.max_msa_depth) if test_records else torch.empty(0)
            else:
                train_embedding = embedder.embed_sequences_per_residue([seq for _, seq in train_records], seq_length=args.seq_length, batch_size=args.batch_size)
                test_embedding = embedder.embed_sequences_per_residue([seq for _, seq in test_records], seq_length=args.seq_length, batch_size=args.batch_size) if test_records else torch.empty(0)

            train_path = write_split_artifact(output_root / "train.pt", fasta_path.stem, "train", train_records, train_embedding, args.model_name, args.embedding_mode)
            test_path = write_split_artifact(output_root / "test.pt", fasta_path.stem, "test", test_records, test_embedding, args.model_name, args.embedding_mode) if test_records else None
        ...
        if args.embedding_mode == "msa":
            embeddings = embedder.embed_msa(sequences, seq_length=args.seq_length, max_msa_depth=args.max_msa_depth)
            artifact_path = write_msa_artifact(output_root / "msa", fasta_path, records, embeddings, args.model_name)
            manifest_rows.append([
                str(fasta_path),
                fasta_path.stem,
                "msa",
            ...
            embeddings = embedder.embed_sequences_per_residue(sequences, seq_length=args.seq_length, batch_size=args.batch_size)
            artifact_path = write_independent_artifact(output_root / "independent", fasta_path, records, embeddings, args.model_name)
            manifest_rows.append([
                str(fasta_path),
                fasta_path.stem,
                "independent",
    ...
    with open(manifest_path, "w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(["source_path", "item_id", "embedding_mode", "artifact_path", "sequence_id"])
